  ...
  if (exists(name)) {
    ...
    if (length(unique(data$PID)) > 0) {
      ...
      try({
        ...
        aic_results_coxme[[paste0("coxme_", name)]] <- AIC(fit)
      }, silent = TRUE)
    }
  }
}

print(aic_results_coxme)
```
...
### Time dependent Cox PH model with Frailty term
```{r}
# List to store average AICs for each dataset
frailty_AIC_results <- list()
...
for (name in dataset_names) {
  ...
  # Average AIC across 5 imputations
  frailty_AIC_results[[name]] <- mean(aic_list, na.rm = TRUE)
}

# Convert results to a dataframe for easy viewing
frailty_AIC_df <- data.frame(
  Dataset = names(frailty_AIC_results),
  Avg_Frailty_AIC = unlist(frailty_AIC_results)
)

# View 
print(frailty_AIC_df)
```
...
### Time dependent Cox PH model with frailty term
```{r}
aic_results_forest_frailty <- list()
...
for (name in dataset_names1) {
    ...
    if (exists(dataset_names1)) {
        ...
        if (inherits(model, "try-error")) {
            aic_results_forest_frailty[[name]] <- NA
            cat("Model failed for:", dataset_names1, "\n")
        } else {
            aic_results_forest_frailty[[name]] <- AIC(model)
        }
    } else {
        cat("Dataset not found:", dataset_names1, "\n")
    }
...
print(aic_results_forest_frailty)
```
